...
def refresh_and_save_data():
    """Czyści CSV i zapisuje świeże dane"""
    new = fetch_new_data()
    if new:
...
def classify_water_levels(data):
    """Klasyfikuje stany wód wg poziomu"""
...
def generate_html_from_csv(csv_file=CSV_FILE, output_file='hydro_table.html'):
    """Generuje kompletną stronę HTML z tabelą, mapą i wykresami"""
    # Wczytaj CSV
    data = []
...
<head>
  ...
  <style>
    ...
    .alarm-summary { background:#e74c3c; }
    .warning-summary { background:#f1c40f; }
    .normal-summary { background:#2ecc71; }
    ...
    .tab-button { padding:10px 20px; background:#eee; border:none; border-radius:5px 5px 0 0;
      cursor:pointer;
    }
    ...
    <div class="table-container">
      <table>
        <thead>
          <tr>
            <th>Kod</th><th>Nazwa</th><th>Współrzędne</th>
            <th>Stan</th><th>Data</th><th>Przepływ</th><th>Data przepływu</th><th>Status</th>
          </tr>
        </thead>
        <tbody>
          {% for r in data %}
          ...
          <tr>
            ...
            <td>
              {% if lvl is not none and lvl >= 500 %}<strong style="color:red">{{ r.stan }}</strong>
              {% elif lvl is not none and lvl >= 450 %}<strong style="color:orange">{{ r.stan }}</strong>
