...
def generate_html_from_csv(csv_file='hydro_data.csv', output_file='hydro_map.html'):
    ...
    with open(csv_file, encoding='utf-8-sig') as f:
        ...
        for row in reader:
            # mapujemy nazwy rzek/akwenow i kod
...
<body>
  <div class="container my-4">
    ...
    <div class="tab-content mt-3">
      ...
      <div class="tab-pane fade show active" id="stats" role="tabpanel">
        ...
        <!-- tutaj tabele alarm, warning, wszystkie -->
      </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Granice Polski
      fetch('/static/poland.geojson').then(r => r.json()).then(geo => L.geoJSON(geo, { style:{color:'#444',weight:2,fillOpacity:0} }).addTo(map));
      // Markery stacji
      const stations = {{ data|tojson|safe }};
      window.stationsLayer = L.layerGroup().addTo(map);
      stations.forEach(p => {
        if (p.lat && p.lon) {
          ...
          const marker = L.circleMarker([p.lat, p.lon], { radius:6, fillColor:color, color:'#000', weight:1, fillOpacity:0.8 });
          marker.bindPopup(`
            <strong>${p.nazwa_stacji}</strong><br>
            Rzeka/Akwen: ${p.river || 'brak'} (${p.riverCode || 'brak'})<br>
            Stan: ${p.stan} m
          `);
          marker.addTo(window.stationsLayer);
        }
      });
      // Wykres poziomów
...
""")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html)
    print('✅ HTML wygenerowany: {}'.format(output_file))
